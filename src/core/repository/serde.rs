...
fn deserialize_contents(content_string: String) -> Vec<File> {
    let content_strings: Vec<&str> = content_string.split(";").collect();
    content_strings
        .into_iter()
        .map(|content_b64| decode_base64(content_b64.to_string()))
        .map(|content| {
            let content_info: Vec<&str> = content.split("|").collect();
            File {
                filename: content_info[0].to_string(),
                content: content_info[1].to_string(),
            }
        })
        .collect()
}
...
fn deserialize_paths(paths_string: String) -> Vec<PathBuf> {
    let paths_strings: Vec<&str> = paths_string.split(";").collect();
    paths_strings
        .into_iter()
        .map(|path| {
            let path_info: Vec<&str> = path.split("|").collect();
            path_info[1]
        })
...
fn serialize_contents(contents: Vec<File>) -> String {
    let contents_strings: Vec<String> = contents
        .into_iter()
        .map(|content: File| format!("{}|{}", content.filename, content.content))
        .map(base64::encode)
        .collect();
...
fn serialize_paths(paths: Vec<PathBuf>) -> String {
    let paths_strings: Vec<String> = paths
        ...
        .map(|path: PathBuf| {
            if path.is_dir() {
                return format!("{}|{}", "dir", path.to_str().unwrap());
            }
            if path.is_file() {
                return format!("{}|{}", "file", path.to_str().unwrap());
            }
